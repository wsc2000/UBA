...
from datasets import get_pretraining_dataset_1, get_pretraining_dataset_2, get_usergroup
...
def local_train(net, data_loader, train_optimizer, epoch, args):
    ...
    for im_1, im_2 in train_bar:
        im_1, im_2 = im_1.cuda(non_blocking=True), im_2.cuda(non_blocking=True)  # Tensor转义到GPU的显存会更快
        feature_1, out_1 = net(im_1)      # feature是提取的特征，out是feature经过映射头的输出
        feature_2, out_2 = net(im_2)
        # [2*B, D]
        out = torch.cat([out_1, out_2], dim=0)  # 拼接，竖着拼
        # [2*B, 2*B]

        sim_matrix = torch.exp(torch.mm(out, out.t().contiguous()) / args.knn_t)
        # .mm()是矩阵相乘； .t()是转置；.contiguous()与原tensor不共享内存，修改不会影响原tensor
        # args.knn_t是softmax temperature in kNN monitor

        # .one_like返回全1的tensor，维度与input相同; .eye()构建单位阵
        mask = (torch.ones_like(sim_matrix) - torch.eye(2 * args.batch_size, device=sim_matrix.device)).bool()
        # [2*B, 2*B-1]
        # 加mask，维度变成 （2 * args.batch_size, 列数由行数决定）
        ...
        # [2*B]
        pos_sim = torch.cat([pos_sim, pos_sim], dim=0)     # 拼接
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_dir', default='./result/fed_pretrain_encoder', type=str, metavar='PATH',
    ...
    parser.add_argument('--frac', type=float, default=0.15, help='the fraction of clients: C')
    ...
    # data_dir = f'E:/coding/Fed_badencoder/data/{args.pretraining_dataset}/'
    ...
    memory_data, test_data_clean = get_pretraining_dataset_2(args)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        data_frame.to_csv(args.results_dir + '/vgg19_cifar10.csv', index_label='epoch')

        if epoch % args.epochs == 0:
            torch.save({'epoch': epoch, 'state_dict': global_model.state_dict(), 'optimizer': optimizer.state_dict(), },
                       args.results_dir + '/cifar10_vgg19' + str(epoch) + '.pth')
