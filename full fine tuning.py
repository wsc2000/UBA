...
if __name__ == '__main__':
    ...
    parser.add_argument('--reference_label', default=9, type=int, help='target class in the target downstream task')
    ...
    parser.add_argument('--trigger_file', default='./trigger/global_trigger.npz', type=str,
    ...
    parser.add_argument('--encoder', default='./result/fed_badencoder/cifar10_backdoor_truck20.pth', type=str, help='path to the image encoder')
    ...
    downstream_classifier = nn.Sequential(model.f, net)
    ...
    for epoch in range(1, args.nn_epochs + 1):
        ...
        parameters = downstream_classifier.parameters()
        optimizer = torch.optim.Adam(parameters, lr=args.lr)

        net_train(downstream_classifier, train_loader, optimizer, epoch, criterion)
        if 'clean' in args.encoder:
            net_test(downstream_classifier, test_loader_clean, epoch, criterion, 'Clean Accuracy (CA)')
            net_test(net, test_loader_backdoor, epoch, criterion, 'Attack Success Rate-Baseline (ASR-B)')
        else:
            ba = net_test(downstream_classifier, test_loader_clean, epoch, criterion, 'Backdoored Accuracy (BA)')
            results['BA'].append(ba)
            asr = net_test(downstream_classifier, test_loader_backdoor, epoch, criterion, 'Attack Success Rate (ASR)')
