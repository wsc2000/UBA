...
from models.foolsgold import foolsgold_aggr
...
def poisoning_train(backdoored_encoder, clean_encoder, data_loader, train_optimizer, poison_ep, args):
    ...
    return total_loss / total_num, backdoored_encoder.state_dict()
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #
        # for i in range(10):
        #     if len(list(idxs_users)) > 8:
        #         idxs_users = list(idxs_users)
        #         idxs_users.pop()
        #     else:
        #         break
        #
        # attacker_list_ = attacker_list
        ...
        global_model_w = copy.deepcopy(global_model.state_dict())
        global_model.train()
        ...
        for idx in idxs_users:
            ...
            for name, data in tqdm(model.state_dict().items()):
                local_updata[name] = (data - global_model_w[name])
            folder_path = f'saved_models/saved_updates'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            update_name = '{0}/update_{1}.pth'.format(folder_path, idx)
            torch.save(local_updata, update_name)
        ...
        if epoch % 1 == 0:
            for attacker in attacker_list_:
                ...
                #         local_weights_ = local_weights
                        # w = average_weights(local_weights)
                #         # model.load_state_dict(w,strict=False)
                if epoch > 1:
                    # local_weights_.reverse()
                    # local_weights_ = local_weights_[0:3]
                ...
                if not os.path.exists(folder_path):
                    os.makedirs(folder_path)
                update_name = '{0}/update_{1}.pth'.format(folder_path, attacker)
                torch.save(local_updata, update_name)
            ...
            for name, data in tqdm(global_model.state_dict().items(),desc='Empty weight_accumulator making'):
                weight_accumulator[name] = torch.zeros_like(data)

            current_users = list(idxs_users) + list(attacker_list_)
            print('Clients in the current round:',current_users)

            weight_accumulator = foolsgold_aggr(weight_accumulator, current_users)


            # temp_model = copy.deepcopy(global_model)

            tem_model_weights = copy.deepcopy(global_model.state_dict())

            for name, sum_weight in weight_accumulator.items():
                scale = 1/10
                average_updata = scale * sum_weight
                # weight_accumulator[name] += average_updata
                tem_model_weights[name] = tem_model_weights[name] + average_updata
                # global_model.state_dict()[name] = global_model.state_dict()[name] + (average_updata)

            # global_model.load_state_dict(tem_model_weights)

            # global_model.load_state_dict(temp_model.state_dict())



            update_w = copy.deepcopy(tem_model_weights)
            ...
            #         local_grad.append(parameters_dict_to_vector_flt(get_grad(w, local_weights_before[10-2])))
            ...
            #     # global_g_norm = torch.norm(global_g)
            #     # print(local_grad_norm)
            #     # print(global_g_norm)
            #
            #
            ...
            #     # local_grad_norm[0] = local_grad_norm[0] * weight_measure[0]
            #     # local_grad_norm[1] = local_grad_norm[1] * weight_measure[1]
            #     # local_grad_norm[2] = local_grad_norm[2] * weight_measure[2]
            #     # local_grad_norm[0] = global_g_norm / local_grad_norm[0]
            #     # local_grad_norm[1] = global_g_norm / local_grad_norm[1]
            #     # local_grad_norm[2] = global_g_norm / local_grad_norm[2]
            #     # # weight_measure = F.normalize(weight_measure)
        ...
        # detect_data_path = 'data/cifar10/server_data_10.npz'
        ...
        #     # detecet_list = StandardScaler().fit_transform(detecet_list)
        #
        #     # copy_list = []
        #     # for i in detecet_list:
        #     #     copy_list.append(i)
        #     # copy_list.sort()
        #
        ...
        if epoch % args.epochs == 0:
            torch.save(
                {'epoch': epoch, 'state_dict': global_model.state_dict(),
                 ...
                 args.results_dir + '/fast_backdoor_testmodel_baseline' + str(epoch) + '.pth'
                     )